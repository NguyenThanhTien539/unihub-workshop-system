  ...
  if (error || !workshop) {
    return (
      <section className="rounded-3xl border border-red-200 bg-red-50 p-6">
        ...
        <h1 className="text-xl font-semibold text-red-900">Không mở được workshop này</h1>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-slate-950">Buổi học và đăng ký</h2>
                  <p className="text-sm text-slate-500">
                    Số chỗ, trạng thái thanh toán và thao tác đăng ký được đồng bộ từ API backend.
                  </p>
                </div>
                {isStudent ? (
              ...
              <div className="mt-4 space-y-4">
                {workshop.sessions.length > 0 ? (
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">Workshop này chưa có buổi học.</div>
                )}
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-950">Trạng thái đăng ký</h2>
              <div className="mt-4 space-y-3 text-sm text-slate-600">
                {!currentUser ? (
                  <>
                    <p>Đăng nhập bằng tài khoản sinh viên để đăng ký, thanh toán và xem vé QR.</p>
                    <Link
                  ...
                  <>
                    ...
                    <p>Mỗi nút buổi học bên dưới phản ánh số chỗ hiện tại và trạng thái đăng ký của bạn.</p>
                    <Link href="/registrations" className="inline-flex text-sm font-medium text-sky-700 hover:text-sky-900">
                      Quản lý đăng ký của tôi
                    </Link>
                  </>
                ) : (
                  <p>Chỉ tài khoản sinh viên được đăng ký workshop. Ban tổ chức và nhân sự check-in có thể xem chi tiết nhưng không thể giữ chỗ.</p>
                )}
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-4 text-sm text-slate-700">
                <InfoRow icon={<Calendar size={18} />} label="Ngày" value={formatSessionDate(firstSession?.startAt)} />
...
function getSessionAction({
  ...
  if (!isStudent) {
    return { kind: "INFO", label: "Chỉ sinh viên được đăng ký", disabled: true };
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4">
      <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
        <h2 className="text-xl font-semibold text-slate-950">
          {mode === "FREE" ? "Xác nhận đăng ký miễn phí" : "Xác nhận đăng ký có phí"}
        </h2>
        <div className="mt-4 space-y-2 text-sm text-slate-600">
          ...
          <p>
            Số chỗ: {formatSeatSummary(session)} · Phí: {formatMoney(session.feeAmount, session.currency ?? "VND")}
          </p>
          {mode === "PAID" ? (
